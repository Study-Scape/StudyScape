...
import BookmarksLoginComponent from '@/components/loginbookmarks';

export default function Page() {
  const [bookmarks, setBookmarks] = useState<Bookmark[] | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [isLoggedIn, setIsLoggedIn] = useState(true); // Add state to track login status
  ...
  const fetchBookmarks = async () => {
    try {
      ...
      setBookmarks(data);
      setIsLoggedIn(true); // Ensure login state is true
    } catch (err) {
      console.error(err);
  ...
  if (!isLoggedIn) {
    return (
      <div className="h-full w-full bg-gradient-to-br from-purple-100 to-indigo-200 p-6">
        <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-6">
          <div className="px-8 py-6">
            <BookmarksLoginComponent />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-full w-full overflow-y-auto scrollbar-hide bg-gradient-to-br from-purple-100 to-indigo-200 p-6">
      <div className="max-w-4xl mx-auto bg-white rounded-xl shadow-2xl p-6">
                ...
                <div key={bookmark.id} className={`mb-6 ${hasImage ? 'grid grid-cols-1 md:grid-cols-3 gap-4' : ''}`}>
                  <div className={`p-6 bg-gray-50 rounded-lg shadow-md border border-gray-200 ${hasImage ? 'md:col-span-2' : 'w-full'}`}>
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {location.hasWifi && <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">WiFi Available</span>}
                      {location.hasFood && <span className="px-3 py-1 bg-green-100 text-green-800 text-xs rounded-full">Food Nearby</span>}
                      {location.hasRestrooms && <span className="px-3 py-1 bg-purple-100 text-purple-800 text-xs rounded-full">Restrooms</span>}
                      {location.hasPrinters && <span className="px-3 py-1 bg-yellow-100 text-yellow-800 text-xs rounded-full">Printers Available</span>}
                    </div>
                    <div className="flex justify-between items-center mb-4">
                      <div>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        {location.hasElevator && <span className="px-3 py-1 bg-orange-100 text-orange-800 text-xs rounded-full">Elevator Available</span>}
                        {location.quietSpaces && <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">Quiet Spaces</span>}
                        {location.hasBikeRack && <span className="px-3 py-1 bg-blue-100 text-blue-800 text-xs rounded-full">Bike Rack Available</span>}
                      </div>
                    </div>
                  </div>
                  {hasImage && (
